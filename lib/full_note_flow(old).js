...
import { transcribeAudio } from "./transcription(old).js";
import os from "os";
...
const LEARNING_STYLES = {
  visual: {
    name: "Visual Learner",
    description:
      "Prefers visual representations like diagrams, charts, and spatial organization",
  },
  auditory: {
  ...
  kinesthetic: {
    ...
    description:
      "Learns through doing, physical activities, hands-on experiences",
  },
};
...
async function retryWithBackOff(
  ...
  while (attempt < retries) {
    try {
      ...
      console.warn(
        `Attempt ${attempt} failed: ${err.message}. Retrying in ${Math.round(
          delay / 1000
        )}s...`
      );
  ...
  try {
    ...
    for (let idx = 0; idx < chunks.length; idx++) {
      ...
      try {
        ...
        console.log(
          `Processing chunk ${idx + 1}/${chunks.length} (${chunkSizeMB.toFixed(
            2
          )} MB)`
        );
        ...
        console.error(
          `❌ Failed to transcribe chunk ${idx + 1}:`,
          error.message
        );
        ...
        if (
          ...
          console.log(
            `Skipping corrupted chunk ${idx + 1} due to format issues`
          );
          fullTranscript += `\n\n[Audio segment ${idx + 1
            } could not be processed due to format issues]\n\n`;
        } else {
...
const full_note_flow = async (noteId) => {
  ...
  try {
    ...
    tempOutputPath = path.join(TEMP_ROOT, `${uuidv4()}_output.txt`);
    ...
    const generatedContent = await generateLectureNotesFlow(
      fullTranscript,
      vark_type
    );
    ...
    if (vark_type === "auditory") {
      ...
      try {
        ...
        // Define the R2 path for the audio file
        audioFilePath = `audioExplanationFile/${uuidv4()}_${name.replace(
          /[^a-zA-Z0-9]/g,
          "_"
        )}.mp3`;

        // Upload the audio buffer to R2
        await r2.send(
        ...
        // Don't fail the whole flow if TTS fails, just log it
        // The audioFilePath will remain null, which is fine
